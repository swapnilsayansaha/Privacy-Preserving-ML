...
int32_t test_millionaire_prob_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	//Basically begins sending money values
	...
	*/
	...
	/**
		Step 9: Executing the circuit using the ABYParty object evaluate the
				problem.
	*/
	auto exec_comp_start = std::chrono::high_resolution_clock::now();
	party->ExecCircuit();
	auto exec_comp_end = std::chrono::high_resolution_clock::now();
	...
	*/
	...
	auto build_dur = build_end - build_start;
	auto exec_dur = exec_comp_end - exec_comp_start;
	auto total_dur = exec_comp_end - transmission_start;
	...
	std::cout << "Circuit build time (ms): " << std::chrono::duration_cast<std::chrono::milliseconds>(build_dur).count() << std::endl;
	std::cout << "Circuit execution time (ms): " << std::chrono::duration_cast<std::chrono::milliseconds>(exec_dur).count() << std::endl;\
	std::cout << "Total Time (ms): " << std::chrono::duration_cast<std::chrono::milliseconds>(total_dur).count() << std::endl;
